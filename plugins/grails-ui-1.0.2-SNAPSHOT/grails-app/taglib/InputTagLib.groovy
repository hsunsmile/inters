...
class InputTagLib {
    ...
    def autoComplete = {attrs ->
        ...
        out << """
                <script>
                    function init_${jsid}_ac() {
                        var cfg = {${grailsUITagLibService.mapToConfig attrs}};
        """
        ...
        out << """
                    GRAILSUI.${jsid} = new YAHOO.widget.AutoComplete('${id}', '${containerId}', dataSource, cfg);
    ...
    def datePicker = {attrs ->
        ...
        """
        // now the script to creates the calendar
        out << """
        <script>
