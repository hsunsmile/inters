...
function Calendar() {
	...
	this.init = function() {
		cal = new YAHOO.widget.Calendar("cal", displayContainer,{ LOCALE_WEEKDAYS:"short", START_WEEKDAY: 1, MULTI_SELECT: false} );
	...
	function showCal() {
    	...
    	if (date) {
    		...
    		if(format == "dd.MM.yyyy"){
	    		var date = date.split('.');    		
		    	tmpDate.setDate(date[0]);
		    	tmpDate.setMonth(date[1]-1);
		    	tmpDate.setYear(date[2]);
    		}
    		else if(format == "MM/dd/yyyy"){
    			var date = date.split('/');    		
		    	tmpDate.setDate(date[1]);
		    	tmpDate.setMonth(date[0]-1);
		    	tmpDate.setYear(date[2]);	
    		}
