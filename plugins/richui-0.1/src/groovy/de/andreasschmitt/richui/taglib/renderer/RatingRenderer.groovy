					...
					if(attrs?.dynamic){
						ul(id: "unit_ul$id", "class": "unit-rating", style: "width: ${units}px"){
							...
							for(i in 1..attrs.units){
								li(){
									String link = attrs.link.replace(":class:", "r${i}-unit rater").replace(":title:", "$i").replace("update", "${id}").replace("number", "$i").replace("id", "$i")
									builder.yieldUnescaped "$link"
									//a("class": "r${i}-unit rater", rel: "nofollow", title: "$i", href:"#", "$i")
								}
							}
						}
					}
