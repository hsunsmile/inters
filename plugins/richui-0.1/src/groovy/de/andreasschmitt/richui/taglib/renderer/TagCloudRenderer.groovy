...
class TagCloudRenderer extends AbstractRenderer {
	...
	protected void renderTagContent(Map attrs, GroovyPageTagBody body, MarkupBuilder builder) throws RenderException {
		...
		builder."div"("class": attrs?.'class', style: attrs?.style){
			form(id:"$tagForm", controller: "${attrs?.controller}", action:"${attrs?.action}", method:"post"){
				if(attrs?.params){
				...
				attrs?.values.each { entry ->					
					...
					a(href: "#", "class":  attrs?.linkClass, style:"font-size: ${fontSize}; $attrs.linkStyle", onclick: 'javascript: document.getElementById("' + selectedTag + '").value = "' + entry.key + '"; document.getElementById("' + tagForm + '").submit();', "${entry.key}")
					if(attrs?.showNumber){
						span(" (${entry.value})")
					}
				}
