...
class SGEPlanMakingAdviseAlgorithm implements AdviseAlgorithm {
	...
	def setBestCluster( Advisor advisor ) {
		...
		synchronized ( clusterObserverIsStarted ) {
			...
			executorService.invokeWithSession("[$threadId][PlanMakingAdviseAlgorithm]") {
				...
				println " [DBG] plan ${selectedPlan} will be used. [${selectionTime}] ";
				planService.executePlan(selectedPlan, selectionTime, "SGEPlanMaking");
				println " ${advisorInst.name} finalized.(${this}) No More Job to serve ";
			}
		}
		println "jobInfoService is : $jobInfoService ";
	}
	...
	def costLimitPlaning( priorKind ) {
		...
		println "\t\t costLimitPlaning ... now test [$priorKind] ";
		...
		planService.makeSGEPlan( priorKind, 'SGEPlanMaking', availableClusters );
	}
	...
	def multiSumbmit() {
		...
		// CHECK
		if( advisorInfoService.jobSubmitionIsFinished() ) {
